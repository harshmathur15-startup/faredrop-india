  ...
  if (locked) {
    return (
      <Link href="/pricing" className="group block rounded-2xl overflow-hidden bg-white shadow hover:shadow-xl transition-all duration-200 hover:-translate-y-1 relative">
        ...
        <div className="relative h-44 w-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-4 py-3">
            ...
            <p className="text-white/70 text-xs flex items-center gap-1.5">
              {deal.airline}
              {cabin && (
                <span className="bg-amber-400 text-amber-950 font-bold px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide">{cabin}</span>
              )}
            </p>
          </div>
        </div>
        ...
        {/* Card body */}
        <div className="p-4">
          {/* Price row — price IS visible, dates are not */}
          ...
          <div className="mt-2 flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
            ...
            <span className="text-sm text-gray-400 select-none">Dates hidden</span>
          </div>

          {/* CTA */}
          <div className="mt-3 flex items-center text-amber-600 text-xs font-semibold group-hover:text-amber-700">
            Upgrade to unlock <span className="ml-1">→</span>
          </div>
        </div>
      </Link>
    )
